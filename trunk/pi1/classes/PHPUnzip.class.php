...
/**
 * @access private
 * Ready to read a local file header
 *
 */
define('S_FILE_HEADER',  1);
/**
 * @access private
 * Ready to read the file header information
 *
 */
define('S_FILE_HEADER_DATA', 2);
/**
 * @access private
 * Ready to read the filename from the file header
 *
 */
define('S_FILE_FILENAME', 3);
/**
 * @access private
 * Check for and read any data in the extra field
 *
 */
...
/**
 * @access private
 * Check for and read from the file data descriptor
 *
 */
...
/**
 * @access private
 * Read the filename from the central directory
 *
 */
define('S_CD_FILENAME', 9);
/**
 * @access private
 * Read extra data from the central directory
 *
 */
define('S_CD_EXTRA', 10);
/**
 * @access private
 * Read the file comment from the central directory
 *
 */
...
/**
 * @access private
 * At end of file or end of zip structure
 *
 */
define('S_EOF', 13);
/**
 * @access private
 * In an error state and will not read anymore
 *
 */
define('S_ERROR', 14);
...
class PHPUnzip {
  ...
  function WriteDataToFile($data, $name, $path, $time = null)
  {
    ...
    if (!is_writeable($this->output_file_path)) { // make sure we can write to the output directory
      ...
      if ($path != '') {
        ...
        foreach($directories as $directory) { // iterate over each directory and concatenate the directory string
          ...
          if (!file_exists( $this->output_file_path . $directory_string ) ) { // haven't created this path yet
            if (!@mkdir($this->output_file_path . $directory_string, 0775)) {
      ...
      fclose($fp);
	  //added by Cyrill Helg
	  array_push($this->createdFiles, array('filename' => $name,'path'=>$directory_string));
// 	  $this->createdFiles[] = array('filename' => $name,'path'=>$filename);
